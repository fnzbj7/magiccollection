...
export class CreateCardVariations1643453448414 implements MigrationInterface {
    name = 'CreateCardVariations1643453448414';

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(
            `CREATE TABLE \`possible_card_variation\` (\`id\` int NOT NULL AUTO_INCREMENT, \`cardVariantType\` enum ('normal', 'etched', 'prerelease', 'stamped', 'list') NOT NULL DEFAULT 'normal', \`has_normal\` tinyint NOT NULL, \`has_foil\` tinyint NOT NULL, \`card_1\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        );
        await queryRunner.query(
        ...
        await queryRunner.query(
            `ALTER TABLE \`possible_card_variation\` ADD CONSTRAINT \`FK_d97d3330508c07f250162278297\` FOREIGN KEY (\`card_1\`) REFERENCES \`card\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
        );
        await queryRunner.query(
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(
            `ALTER TABLE \`possible_card_variation\` DROP FOREIGN KEY \`FK_d97d3330508c07f250162278297\``,
        );
