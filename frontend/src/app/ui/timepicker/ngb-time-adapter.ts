...
const NGB_DATEPICKER_TIME_ADAPTER_FACTORY = () => new NgbTimeStructAdapter();
export default NGB_DATEPICKER_TIME_ADAPTER_FACTORY;
    ...
    fromModel(time: NgbTimeStruct | null): NgbTimeStruct | null {
        return time && isInteger(time.hour) && isInteger(time.minute)
            ? {
                  hour: time.hour,
                  minute: time.minute,
                  second: isInteger(time.second) ? time.second : 0,
              }
            : null;
    }
