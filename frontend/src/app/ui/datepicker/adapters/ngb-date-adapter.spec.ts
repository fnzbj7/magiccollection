...
describe('ngb-date model adapter', () => {
    ...
    describe('fromModel', () => {
        ...
        it('should bypass numeric date', () => {
            expect(adapter.fromModel({ year: 0, month: 0, day: 0 })).toEqual({
                year: 0,
                month: 0,
                day: 0,
            });
            expect(adapter.fromModel({ year: 2016, month: 5, day: 1 })).toEqual({
                year: 2016,
                month: 5,
                day: 1,
            });
    ...
    describe('toModel', () => {
        ...
        it('should bypass numeric date', () => {
            expect(adapter.toModel({ year: 0, month: 0, day: 0 })).toEqual({
                year: 0,
                month: 0,
                day: 0,
            });
            expect(adapter.toModel({ year: 2016, month: 10, day: 15 })).toEqual({
                year: 2016,
                month: 10,
                day: 15,
            });
